...
@pytest.mark.asyncio
async def test_po_match_loads_master_form_id_for_any_tenant():
    invoice = "invoice-form"
    master = "po-master-form"
    looked_up: list[str] = []
    ...
    class Ez:
        ...
        async def lookup_po(self, **kwargs):
            looked_up.append(kwargs.get("form_id"))
            return {
                "po_number": "PO-1",
                "vendor": "ACME",
                "total": 100.0,
    ...
    result = await run_po_match(ctx)
    assert looked_up == [master]
    assert ctx.document_job["master_form_id"] == master
    assert "refusing" not in (result.data.get("reason") or "").lower()
