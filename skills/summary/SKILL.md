...
2. First infer the document type from the text or JSON (invoice, insurance policy/claim/certificate, purchase order, contract, letter, report, ID, receipt, or other). Never call it an invoice unless the source clearly supports that.
...
The user message will include the document source label and either OCR text or JSON data. Follow any additional rules attached below (including key-facts count and highlight limits).
